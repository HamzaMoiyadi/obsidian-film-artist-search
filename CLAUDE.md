...
- **Target both desktop and mobile.** `isDesktopOnly` must remain `false`.
- Test on iOS and Android where feasible.
- Do not use any Node/Electron-only APIs. All network calls (TMDb, etc.) must use `requestUrl` from the Obsidian API, not Node's `fetch` or `https` module, to ensure mobile compatibility.
